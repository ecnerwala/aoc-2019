...
def main(inp, is_real):
    ...
    comps = [Computer(prog).run_with_inp([i]) for i in range(50)]
        ...
        for i,comp in enumerate(comps):
            ...
            if not comp.halted:
                if q[i]:
                    is_idle = False
                    x,y = q[i].pop(0)
                    comp.run_with_inp([x,y])
                else:
                    comp.run_with_inp([-1])
            if comp.has_output(): is_idle = False
        if is_idle:
            if last == (nat_x, nat_y):
                cprint(nat_y)
                return
            last = (nat_x, nat_y)
            q[0].append((nat_x, nat_y))
    cprint(q[255][0][1])
    #exit(0)
